...
این محدودیت مهمی است: نتیجه `obj1 + obj2` (یا عملیات ریاضی دیگر) نمی‌تواند شیء دیگری باشد!
...
پس چون نمی‌توانیم از نظر فنی اینجا کار خاصی کنیم، در پروژه‌های واقعی شیءها همراه با ریاضیات استفاده نمی‌شوند. زمانی که اتفاق می‌افتد، به جز استثناءهای کمیاب، معمولا بخاطر یک اشتباه کدنویسی است.
...
1. تبدیل به بولین وجود ندارد. در زمینه بولین تمام شیءها `true` هستند، به همین سادگی. فقط تبدیل عددی و رشته‌ای وجود دارد.
...
حال بیایید به جزئیات فنی بپردازیم چون تنها راهی که موضوع را عمیقا پوشش دهیم همین است.

# جزءها (Hints)

جاوااسکریپت چگونه تصمیم می‌گیرد که کدام تبدیل را اعمال کند؟
...
: برای تبدیل شیء به عدد، مثل زمانی که ما از ریاضی استفاده می‌کنیم:
    ...
    اکثر تابع‌های ریاضیاتی درون‌ساخت هم چنین تبدیل‌هایی را شامل می‌شوند.

`"default"`
: در موارد کمیاب زمانی که عملگر «مطمئن نیست» که چه نوعی را دریافت می‌کند.

    برای مثال، عملگر مثبت دوگانه `+` هم می‌تواند با رشته‌ها کار کند (آن‌ها را ادغام کند) و هم با عددها (آن‌ها را به یکدیگر اضافه کند). پس اگر یک مثبت دوگانه شیءای را دریافت کند، از جزء `"default"` برای تبدیل آن استفاده می‌کند.
...
تمام شیءهای درون‌ساخت به جز یک مورد (شیء `Date`، بعدا آن را یاد خواهیم گرفت) تبدیل `"default"` را مانند `"number"` پیاده‌سازی می‌کنند. و احتمالا ما باید همین کار را کنیم.

اگرچه اینکه درباره تمام 3 جزء بدانیم مهم است، به زودی دلیل آن را خواهیم دید.
...
همانطور که از کد می‌بینیم، `user` به رشته‌ای خودتعریف‌کننده یا مقداری پول تبدیل می‌شود، بسته به نوع تبدیل. متد `user[Symbol.toPrimitive]` به تنهایی تمام موارد تبدیل را به عهده می‌گیرد.
...
- برای جزء `"string"`: متد `toString` را فراخوانی کن و اگر این متد وجود نداشت، سپس `valueOf` (پس `toString` برای تبدیل‌های رشته‌ای اولویت دارد).
...
در مقابل، `Symbol.toPrimitice` سخت‌گیرتر است و *باید* یک مقدار اصلی برگرداند، در غیر این صورت یک ارور خواهیم داشت.
```
...
اگر ما به عنوان آرگومان شیءای را پاس دهیم، دو مرحله محاسبات وجود خواهد داشت:
1. شیء به یک مقدار اصلی تبدیل می‌شود (با استفاده از قوانینی که بالاتر گفتیم).
2. اگر مقدار اصلی حاصل، از نوع درستی نباشد، دوباره تبدیل می‌شود.
...
- `"number"` (برای ریاضیات)
- `"default"` (برای عملگرها، معمولا شیءها آن را درست مانند `"number"` پیاده‌سازی می‌کنند).
...
تمامی این متدها باید یک مقدار اصلی را برای کار کردن برگردانند (اگر تعریف شده باشد).

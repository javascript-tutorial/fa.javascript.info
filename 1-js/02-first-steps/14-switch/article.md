...
این توانایی که می‌توان caseهای مختلف را گروه‌بندی کرد، عارضه‌ای جانبی از نحوه کارکرد `switch/case` بدون `break` است. اینجا اجرای کد `case 3` از خط `(*)` شروع می‌شود و تا خط `case 5` ادامه پیدا می‌کند چون هیچ `break` وجود ندارد.

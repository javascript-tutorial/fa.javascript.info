...
یک [متغیر](https://en.wikipedia.org/wiki/Variable_(computer_science)) (variable) "حافظه‌ای نام گذاری شده" برای داده است. ما می‌توانیم از متغیرها برای ذخیره کالاها، بازدید کننده‌ها و داده‌های دیگر استفاده کنیم.
...
جالب است بدانید زبان‌های [functional](https://en.wikipedia.org/wiki/Functional_programming) (تابع‌محور) مانند [Scala](http://www.scala-lang.org/) یا [Erlang](http://www.erlang.org/) تغییر مقدار متغیر را ممنوع کرده‌اند.
...
زمانی‌ که یک نام متشکل از چند کلمه است، عموما از روش [camelCase](https://en.wikipedia.org/wiki/CamelCase) استفاده می‌شود. به این صورت که کلمات یکی پس از دیگری پشت هم نوشته می‌شوند و حرف اول هر کلمه حرف بزرگ است: `myVeryLongName`.
...
```js run no-strict
// در این مثال نیست "use strict" در نظر داشته باشید: هیچ

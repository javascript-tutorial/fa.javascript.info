...
به دلیل اینکه این عملگر با `null` و `undefined` به طور یکسان رفتار می‌کند، ما در این فصل از یک اصطلاح خاص استفاده می‌کنیم. برای سادگی، می‌گوییم که یک عبارت "تعریف شده" است هرگاه که نه `null` باشد و نه `undefined`.
...
یک مورد متداول برای استفاده از `??` تعیین کردن مقداری پیش فرض است.
...
ما می‌خواهیم اسم کاربر را با استفاده از یکی از این متغیرها نمایش دهیم، یا اگر تمام آن‌ها `null/undefined` باشند "Anonymous" را نمایش  دهیم.
...
پس شاید نیاز داشته باشیم که در عباراتی مانند این، پرانتز اضافه کنیم:
...
// اینگونه کار می‌کند (شاید چیزی که ما می‌خواهیم نباشد)...
let area = height ?? (100 * width) ?? 50;
```
